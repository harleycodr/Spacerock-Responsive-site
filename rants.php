...
if($link)
{
			...
			if($result2=mysql_query("SELECT
				...
				order by date DESC LIMIT 15"))
				{
					while($row=mysql_fetch_array($result2))
					{
						...
						$leftcol .= "<p><a href=\"rants.php?r=$r\">$title</a></p>\n";
					}
				}
				else
				{
					$goof=mysql_error();
					$error = "<b>Error:</b> $goof Line 66";
	...
	else // show whole list
	{
		...
		else
		{
			...
			$offset_limit="LIMIT 5 OFFSET $offset";
		}
		
		if($result2=mysql_query("SELECT ID from rants"))
		{
		...
		if($prev_page>0)
		{
			$pagination_links.="<a href=\"rants.php?page=$last_page\"><</a> ";
		...
		{
			$pagination_links .= "<a href=\"rants.php?page=$pagination_counter\">$pagination_counter</a>  ";
		...
		$pagination_links .= "<b>$this_page</b>  ";
			...
			while($pagination_counter<$last_page_limiter)
			{
			  ...
			  $pagination_links.="<a href=\"rants.php?page=$next_page\" title=\"next\">></a>";
			}	
		else
		{
			$goof=mysql_error();
...
if($listall=="1")
{
	print "<h1 class=\"tac\">All Opinion Items</h1>\n";
	print "<p>$pagination_links</p>\n";
}
else
{
	print "<p class=\"breadcrumb\"><a href=\"rants.php\">Opinion</a> > $thistitle</p>\n";
}
print <<<ENDTAG
